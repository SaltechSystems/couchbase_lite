...
class Replicator {
  ...
  static const EventChannel _replicationEventChannel = const EventChannel("com.saltechsystems.couchbase_lite/replicationEventChannel");
  final Stream _replicationStream = _replicationEventChannel.receiveBroadcastStream();
  ...
  ListenerToken addChangeListener(ListenerCallback callback) {
    ...
    tokens[token] = _replicationStream.where((data) => data["replicator"] == replicatorId).listen((data) {
      ...
      callback(ReplicatorChange(this, ReplicatorStatus._internal(activity, error)));
    });
    return token;
  }
